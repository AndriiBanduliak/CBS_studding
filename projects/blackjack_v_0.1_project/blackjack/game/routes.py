...
def place_bet():
    try:
        ...
        return jsonify(success=False, error="Invalid bet amount format")
...
def save_game_result(game):
    ...
    try:
        ...
        # Update session with latest game state
        session['game'] = game_to_dict(game)
        ...
        db.session.rollback()
        return jsonify(success=False, error=f"Failed to save game result: {str(e)}")
